        ...
        internal fun setNextFrame(data: ByteArray, camera: Camera) {
            synchronized(lock) {
                ...
                if (!bytesToByteBuffer.containsKey(data)) {
                    d { "Skipping frame. Could not find ByteBuffer associated with the image data from the camera." }
                    return
                }
        ...
        override fun run() {
            ...
            while (true) {
                synchronized(lock) {
                    while (active && pendingFrameData == null) {
                        try {
                            ...
                            e(e) { "Frame processing loop terminated." }
                            return
                        }
                    }
                ...
                try {
                    ...
                    e(t) { "Exception thrown from receiver." }
                } finally {
                    data?.let {
                        camera?.addCallbackBuffer(it.array())
                    }
